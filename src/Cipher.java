    ...
    public static String encrypt(String text, int shift) throws IOException {
        if (Validator.isValidKey(shift)) {
            ...
            for (int i = 0; i < text_in.length; i++) {
                ...
                if (ALPHABET.length > index + shift) {
                    text_in[i] = ALPHABET[index + shift];
                } else {
                    text_in[i] = ALPHABET[(index + shift) - ALPHABET.length];
                }
            }
    ...
    public static String decrypt(String encryptedText, int shift) throws IOException {
        if (Validator.isValidKey(shift)) {
            ...
            for (int i = 0; i < text_in.length; i++) {
                ...
                if (0 <= index - shift && ALPHABET.length > index - shift) {
                    text_in[i] = ALPHABET[index - shift];
                } else {
                    text_in[i] = ALPHABET[(index - shift) + ALPHABET.length];
                }
